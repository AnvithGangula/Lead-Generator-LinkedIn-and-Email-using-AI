...
# --- CONFIG & INIT ---
st.set_page_config(page_title="FusionX Founder Control Center", page_icon="🎯", layout="wide")
init_db()
...
if 'focus_prospect_id' not in st.session_state: st.session_state.focus_prospect_id = None
if 'focus_source' not in st.session_state: st.session_state.focus_source = None
...
mode = st.sidebar.radio("Select System", ["LinkedIn System", "Email Automation", "Master Database"])
st.sidebar.divider()
...
if mode == "LinkedIn System":
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["🚀 Upload", "📂 All IDs", "🔥 Follow-ups", "🆕 New Leads", "📊 Table"])
        ...
        if file and st.button("Generate 8-Part Sequences"):
            ...
            for _, row in df.iterrows():
                ...
                if name and not is_duplicate(name, comp):
                    seq = generate_vamshi_sequence({'Name': name, 'Company': comp, 'Industry': ind})
    ...
    with tab2:
        ...
        if search_q: query += f" AND (name LIKE '%{search_q}%' OR company LIKE '%{search_q}%')"
        p_df = pd.read_sql_query(query, conn)
        if not p_df.empty:
            sel = st.selectbox("Select Prospect", p_df.apply(lambda r: f"{r['name']} | {r['company']}", axis=1))
            p_row = p_df[p_df.apply(lambda r: f"{r['name']} | {r['company']}", axis=1) == sel].iloc[0]
            st.link_button(f"🔗 View {p_row['name']}'s Profile", p_row['linkedin'])
            msgs = pd.read_sql_query(f"SELECT id, stage, content, status FROM messages WHERE prospect_id = {p_row['id']} AND channel='linkedin'", conn)
            for _, row in msgs.iterrows():
                with st.expander(f"{'✅' if row['status'] == '1' else '⏳'} {row['stage']}"):
                    st.code(row['content'], language=None)
                    if st.button("Mark as Sent", key=f"li_sent_{row['id']}"):
                        update_message_status(row['id'], '1'); st.rerun()
        conn.close()
    ...
    with tab3:
        ...
        if due_df.empty:
            ...
            for _, row in due_df.iterrows():
                if st.button(f"👉 Get Message for {row['name']} ({row['stage']})", key=f"alert_{row['id']}"):
                    st.session_state.focus_prospect_id = row['prospect_id']; st.session_state.focus_source = "tab3"; st.rerun()
        conn.close()

    with tab4:
        conn = sqlite3.connect("data/fusionx_unified.db")
        new_df = pd.read_sql_query("SELECT * FROM prospects WHERE id NOT IN (SELECT DISTINCT prospect_id FROM messages WHERE status = '1' AND channel='linkedin') AND linkedin != ''", conn)
        if new_df.empty:
            ...
            for _, row in new_df.iterrows():
                with st.container(border=True):
                    ...
                    if st.button("🚀 Start Outreach", key=f"start_{row['id']}"):
                        st.session_state.focus_prospect_id = row['id']; st.session_state.focus_source = "tab4"; st.rerun()
        conn.close()

    with tab5:
        conn = sqlite3.connect("data/fusionx_unified.db")
        master_df = pd.read_sql_query("SELECT name, company, industry, linkedin FROM prospects WHERE linkedin != ''", conn)
...
# ================== MODE 2: EMAIL AUTOMATION (TABBED UI) ==================
    ...
    tab_dash, tab_pros, tab_up, tab_run, tab_gen, tab_track, tab_settings = st.tabs([
        "📊 Dashboard", "👥 Prospects", "📤 Upload", "⚡ Run Pipeline", "✉️ Generated", "📅 Follow Tracker","⚙️ Settings"
    ])
    ...
    with tab_dash:
        try:
            ...
            total_p = conn.execute("SELECT COUNT(*) FROM prospects WHERE email != ''").fetchone()[0]
            sent_e = conn.execute("SELECT COUNT(*) FROM messages WHERE channel='email' AND status='sent'").fetchone()[0]
            pending_e = conn.execute("SELECT COUNT(*) FROM messages WHERE channel='email' AND status='0'").fetchone()[0]
            ...
            if not df_summary.empty:
                ...
                st.info("No outreach data found. Generate sequences to see the summary.")
    ...
    with tab_pros:
        ...
        df_p = pd.read_sql_query("SELECT company, name, email, industry FROM prospects WHERE email != ''", conn)
    ...
    with tab_up:
        ...
        with col_up1:
            ...
            file = st.file_uploader("Upload Excel/CSV", type=["xlsx", "csv"], key="bulk_email")
...
# --- 4. RUN PIPELINE ---
    with tab_run:
        ...
        targets = pd.read_sql_query("SELECT id, name, company FROM prospects WHERE email != '' AND email IS NOT NULL", conn)
        ...
        if not targets.empty:
            ...
            with col1:
                unique_companies = ["— All Companies —"] + sorted(targets["company"].unique().tolist())
            ...
            with col2:
                ...
                if sel_company == "— All Companies —":
                    employee_list = ["— All Employees —"] + sorted(targets["name"].unique().tolist())
                else:
                    employee_list = ["— All Employees —"] + sorted(targets[targets["company"] == sel_company]["name"].tolist())
            ...
            if st.button("🚀 Generate Emails", use_container_width=True):
                if df_to_gen.empty:
                    ...
                    for index, p in enumerate(df_to_gen.iterrows()):
                        p_data = p[1]
                        status_text.text(f"Processing {index+1}/{len(df_to_gen)}: {p_data['name']} ({p_data['company']})")
                        
                        # Logic to skip if already generated
                        if skip_gen:
                            existing = conn.execute("SELECT COUNT(*) FROM messages WHERE prospect_id=? AND channel='email'", (p_data['id'],)).fetchone()[0]
                            if existing > 0:
                                progress_bar.progress((index + 1) / len(df_to_gen))
                                continue

                        # Call your existing AI sequence function
                        seq = get_8_step_email_sequence(p_data['name'], p_data['company'])
                        
                        for step in seq:
                            conn.execute("""
                                INSERT INTO messages (prospect_id, channel, stage, subject, content, status) 
                                VALUES (?, 'email', ?, ?, ?, '0')
                            """, (p_data['id'], step['label'], step['sub'], step['body']))
                        
                        conn.commit()
                        progress_bar.progress((index + 1) / len(df_to_gen))
                    
                    status_text.empty()
                    st.success(f"Successfully generated sequences for {len(df_to_gen)} leads!")
        else:
            st.info("No email prospects found in the master database. Please upload leads first.")
...
# --- 5. GENERATED EMAILS ---
    with tab_gen:
        ...
        if not df_raw.empty:
            ...
            f_comp = c1.selectbox("Filter Company", ["All"] + sorted(df_raw["company"].unique().tolist()), key="gen_c")
            ...
            if f_comp != "All": 
                df_filtered_view = df_filtered_view[df_filtered_view["company"] == f_comp]
            if f_name != "All": 
            ...
            with st.expander("🚀 Bulk Sending Options", expanded=True):
                ...
                selected_bulk_stage = bulk_col1.selectbox("Select Sequence Step to Send", filtered_stages)
                
                bulk_targets = df_filtered_view[df_filtered_view["stage"] == selected_bulk_stage]
                button_label = f"Send {len(bulk_targets)} Mails"
                
                if bulk_col2.button(button_label, use_container_width=True, type="primary"):
                    if not user_mail or not app_pass:
                        ...
                        for i, (_, row) in enumerate(bulk_targets.iterrows()):
                            if send_email_smtp(row['p_email'], row['subject'], row['content'], user_mail, app_pass):
            ...
            opts = [f"{r['name']} ({r['company']}) - {r['stage']}" for _, r in df_filtered_view.iterrows()]
            sel = st.selectbox("Select Draft to Review", opts, index=st.session_state.idx)
            row = df_filtered_view.iloc[opts.index(sel)]
            
            with st.container(border=True):
                st.write(f"**To:** {row['p_email']}")
                edit_sub = st.text_input("Subject", row['subject'], key=f"edit_sub_{row['id']}")
                edit_body = st.text_area("Body Content", row['content'], height=250, key=f"edit_bod_{row['id']}")
                
                if st.button("📤 Send Individual Email"):
                    if send_email_smtp(row['p_email'], edit_sub, edit_body, user_mail, app_pass):
                        update_message_status(row['id'], 'sent')
                        st.success(f"Sent to {row['name']}!")
                        # Advance index for easier reviewing
                        st.session_state.idx = (st.session_state.idx + 1) % len(df_filtered_view)
                        time.sleep(1); st.rerun()
        else:
...
# --- 7. SMTP SETTINGS (New Tab) ---
    with tab_settings:
        ...
        **Setup Instructions:**
        ...
        res_user = conn.execute("SELECT value FROM settings WHERE key='smtp_user'").fetchone()
        res_pass = conn.execute("SELECT value FROM settings WHERE key='smtp_pass'").fetchone()
        
        saved_user = res_user[0] if res_user else ""
        saved_pass = res_pass[0] if res_pass else ""

        with st.form("smtp_config_form"):
            new_user = st.text_input("Sender Gmail", value=saved_user, placeholder="example@gmail.com")
            new_pass = st.text_input("App Password", value=saved_pass, type="password")
            
            if st.form_submit_button("💾 Save Credentials"):
                if "@" in new_user and len(new_pass) >= 16:
                    conn.execute("INSERT OR REPLACE INTO settings (key, value) VALUES ('smtp_user', ?)", (new_user,))
                    conn.execute("INSERT OR REPLACE INTO settings (key, value) VALUES ('smtp_pass', ?)", (new_pass,))
                    conn.commit()
        ...
        test_to = st.text_input("Recipient Email for Test", placeholder="test@example.com")
            ...
            if not saved_user or not saved_pass:
                ...
                with st.spinner("Testing connection..."):
                    ...
                    if success:
                        ...
                        st.error("Connection failed. Check your App Password or Internet connection.")
...
# ================== MODE 3: MASTER DATABASE ==================
    ...
    conn = sqlite3.connect("data/fusionx_unified.db")
    master_df = pd.read_sql_query("SELECT id, name, company, industry, email, linkedin, created_at FROM prospects", conn)
    st.dataframe(master_df, use_container_width=True, hide_index=True)
    conn.close()