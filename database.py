...
def init_db():
    ...
    conn.execute(
        ...
        CREATE TABLE IF NOT EXISTS messages (
            ...
            UNIQUE(prospect_id, channel, stage)
        )
    """
    )
...
def update_linkedin_status(msg_id, status="1"):
    try:
        ...
        # Get prospect_id for this message
        row = cursor.execute(
            "SELECT prospect_id FROM messages WHERE id = ?", (msg_id,)
        ).fetchone()
        ...
        # Also update next_followup on the NEXT unsent linkedin message
        if row:
            prospect_id = row[0]
            cursor.execute(
                """
